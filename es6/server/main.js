...
const mmdbreader = mmdbreader_override.mmdbreader;
...
const IpGeo_ = ((mtr) => {
  ...
  class IpGeo_ {
    ...
    static load(databaseUrl) {
      const geocoderSettings = mtr.settings && mtr.settings.IpGeo;
      const url = databaseUrl || (geocoderSettings && geocoderSettings.databaseUrl) || _defaultDatabaseUrl;
      // const self = this;
  ...
  // Methods
  mtr.methods({
    'BasMTR:IpGeo:geocode' (ip, verbose) {
      this.unblock();
      if (!ip) {
